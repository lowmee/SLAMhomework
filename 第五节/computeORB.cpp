...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        if(u>=half_patch_size && v>=half_patch_size && u<(image.rows-half_patch_size) && v<(image.cols-half_patch_size)){
            for(int i=-8; i<=7; i++){
                for(int j=-8; j<=7; j++){
                    I=image.at<uchar>(u+i,v+j);
                    m01 += (v+i)*I;
                    m10 += (u+j)*I;
                }
            }
        ...
        kp.angle = (float)(atan2(m01,m10)/pi*100); // compute kp.angle
